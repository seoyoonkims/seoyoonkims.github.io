...
**코드**  
> Github Link: <https://github.com/seoyoonkims/Autonomous-Rover>
...
**1. Localization**  
  ...
  Localization이란 Map의 고정된 global frame을 동적인 local body frame으로 Transformation 하는 것이다.  
...
```
    if (!bGetTarget){
      ...
      vel_command.angular.z = 0.1 * (proportional_y + integral_term_y + proportional_yaw + 
                                                    integral_term_yaw + derivative_term_yaw);
      msg.data = "Navigation";
